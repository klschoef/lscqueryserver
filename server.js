    ...
    if (keys.includes("altitudes")) {
        if (Array.isArray(queryInput.altitudes)) {
            ...
            for (k=0; k < queryInput.altitudes.length; k++) {
                let subquery = {altitude: {$gt: queryInput.altitudes[k]-0.1, $lt: queryInput.altitudes[k]+0.1}};
                console.log(subquery);
                subqueries.push(subquery);
            }
            let partQuery = {$or: subqueries};
            ...
            let partQuery = {altitude: {$gt: queryInput.altitudes-0.1, $lt: queryInput.altitudes+0.1}};
